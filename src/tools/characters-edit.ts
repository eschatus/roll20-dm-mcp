...
// object (name, bio, gmnotes, avatar, controlledby, archived, inplayerjournals).
...
export function registerCharacterEditTools(server: McpServer): void {
  server.tool(
    ...
    [
      ...
      "Supports: name, bio, gmnotes (GM-only HTML), avatar (Roll20 CDN url),",
      "controlledby (player ids or 'all'), archived (boolean),",
      "inplayerjournals (player ids or 'all' or '').",
    ...
    ].join(" "),
    {
      ...
      gmnotes: z.string().optional().describe("GM-only notes HTML"),
    ...
    async ({ charId, name, bio, gmnotes, avatar, controlledby, archived, inplayerjournals }) => {
