...
//
// Drives the REAL combat MCP tool handlers against the emulator through the bridge
// seam, exercising a full round over a mixed warband:
//   initiative · spell/skill sheet reads · AoE (Fireball zone + range) · emanation
//   (Spirit Guardians aura) · multi-target damage · conditions/markers · death →
//   map layer · healing · narration · advance.
//
// No LLM and no D&D Beyond: tactics planning moved to the gem (#171) and the DDB
// bridge is on its way out, so this suite is deterministic by construction rather
// than by mocking.
// ─────────────────────────────────────────────────────────────────────────────
