...
| 1.5 | **Deploy the Mod relay BY HAND**: open the campaign's API/Mod console, paste `mod-scripts/ai-relay.js`, save ⚠ | Mod console prints `[GM_AI_Bridge] Relay script loaded (v2.6.2)`. **Verify the LOAD, not the save** — a saved-but-crashed script looks identical if you only check the save. Deploys are **per-campaign**: a campaign you haven't pasted into is running an old relay, or none. | [ ] |
...
| 1.7 | (A) "transport status" → `transport_status` 🔒 | RT healthy; **circuit breaker closed**; counters present; active campaign = e2e-test; **relay version handshake reports 2.6.2** with no mismatch warning (a mismatch means 1.5 didn't take) | [ ] |
...
- **Mod redeploy after editing `mod-scripts/ai-relay.js`:** **paste it into the
  ...
  `[GM_AI_Bridge] Relay script loaded (v2.6.2)`. Verify the LOAD, not the save.
