...
- The Gem pins this repository as a dependency by tag (currently `#v2.0.5`) and builds it in the clone, so you do not need a separate checkout of roll20-dm-mcp for the Gem to run. Changes here reach the Gem only when it re-pins.
