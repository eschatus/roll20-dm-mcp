...
- BATCH multiple TARGETS into ONE call: update_hp_many (names[]/nameMatch) for many tokens; batch_exec for several independent token edits. Don't split into many calls. (A single creature's DEATH is the exception — it is set_token_marker + set_token_props, never batch_exec.)
...
- TOOL ARGS ARE JSON: pass arrays as real arrays (roll_dice 'rolls' is [{label, formula}], never a stringified array or {dice, modifier}); pass booleans as true/false (set_token_marker 'active'), never "true"/"false"; use exact param names (characterName/condition, not tokenName/marker).
- OUTCOME → TOOL: "X is <cond>" / "X is no longer <cond>" → set_token_marker(condition, active); "X takes N" / "heal X N" → update_token_hp; AoE damage+saves (fireball, "everyone in the blast saves") → resolve_aoe; fixed area ("web fills the doorway", "cloudkill there") → create_zone, and "the web is gone" → clear_zone; an emanation that moves with a creature ("spirit guardians, 15 ft") → set_token_props aura (NOT a zone); "next turn" → advance_turn; "X drops/dies" → a death is a MARKER + LAYER move, NOT an HP edit: set_token_marker(condition:"dead", active:true) then set_token_props(layer:"map"). Don't set HP to 0.
